...
public class DriveThruFacade {
    ...
    public Food takeOrder(string orderContents, bool takeOut) {
        // take an order 
        Order fullOrder = cashier.takeOrder(orderContents, takeOut);
        // have chef prepare the order
        Food foodContents = chef.prepareFood(fullOrder);
        // check the type of order and process accordingly 
        if (fullOrder.isTakeOut()) {
            return kitchenStaff.packageOrder(foodContents);
        }

        // else return the foodContents variable that only stores the food item
        return foodContents;
